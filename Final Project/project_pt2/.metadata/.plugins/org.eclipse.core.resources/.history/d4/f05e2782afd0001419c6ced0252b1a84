...
int i=0;
float *M1,*M2,*M3,*M4,*N,*N_i,*P,*residue,*mag;
complex *buffer,*buffer_out,*frame_lag1,*frame_lag2;
int 	*residue_red0,*residue_red1;
...
float	tau2=0.08;
int 	alpha=	2;//trade-off between speech and noise
float   lambda=0.05;//trade-off between background noise and musical noise, ALLOWS LOWER in this example
...
/*  Initialize and zero fill arrays */  
	...
	buffer_out	= (complex *) calloc(FFTLEN, sizeof(complex));
	
	residue_red0= (int *) calloc(FFTLEN, sizeof(int));
	residue_red1= (int *) calloc(FFTLEN, sizeof(int));
		
	residue		= (float *) calloc(FFTLEN, sizeof(float));	
	
	frame_lag1	= (complex *) calloc(FFTLEN, sizeof(complex));
	frame_lag2	= (complex *) calloc(FFTLEN, sizeof(complex));
...
void noise_estimate(){
	...
	{          
		...
		residue_red1[i]=residue_red0[i];//delay commands
		residue[i]=abs(N_i[i]-N[i]);//determine current residue
		residue_red0[i]=0;
		if(cabs(buffer[i])<residue[i]){
			residue_red0[i]=1;		//1 is the command to choose minimum among adjacent frames
		}
	}	
}
...
void noise_reduction(){
	...
	{   
		...
		buffer_out[k]=frame_lag1[k];
		if(residue_red1[k]==1){
			if(mag<cabs(buffer_out[k])){
			buffer_out[k]=frame_lag1[k];
			}
			if(cabs(frame_lag2[k])<cabs(buffer_out[k])){
			buffer_out[k]=frame_lag2[k];
			}			
		}
		frame_lag2[k]=frame_lag1[k];
		frame_lag1[k]=buffer[k];
		buffer[k] = rmul(gain,buffer_out[k]);
	} 
	ifft(FFTLEN,buffer_out);
}
