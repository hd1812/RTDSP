...
#define FRAMEINC (FFTLEN/OVERSAMP)	/* Frame increment *///overlap area is 3/4 of total
...
int 	lpf_N_switch=1;
int		oversubtraction;
float 	tau=0.02;//Unit ms, range 20~80 ms, control weight of latest input
float 	alpha=	2;//trade-off between speech and noise
float   lambda=0.15;//trade-off between background noise and musical noise
 /******************************* Function prototypes *******************************/
...
void frame_in(){
	...
	if(i==0){
		...
		for(k=0;k<FFTLEN;k++){
			M1[k]=(1-lpf_weight)*mag[k]+lpf_weight*M1[k];
		}
	}
}
...
void noise_estimate(){
	...
	{                          
		...
		if(N_i[i]>M4[i]){N_i[i]=M4[i];}
			...
			//failed oversubtraction
			//alpha=-0.15*10*(-(-mag[i]/N_i[i]+2))/2.3+4;
			//if(alpha>5){alpha=5;}
			//if(alpha<1){alpha=1;}
		if((mag[i]-N_i[i])>N_i[i]){
			alpha=2;
		}
		else{
			alpha=5;
		}
		N[i]=alpha*(1-lpf_weight)*N_i[i]+lpf_weight*N[i];
	}	
}
