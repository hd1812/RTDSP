...
int i=0;
float *M1,*M2,*M3,*M4,*N,*P,*N_i;
complex *buffer;
float lpf_weight;
int 	nc=1;
int 	lpf_switch=1;
int 	lpf_power_switch=0;
int 	lpf_N_switch=1;
float 	tau=0.02;//Unit ms, range 20~80 ms
float 	alpha=	2;
float 	lambda=0.1;//trade off between musical noise and broadband noise
int 	gain_sel=0;
int 	oversubtraction=1;
int 	rotate_inteval=10;
 /******************************* Function prototypes *******************************/
...
/*  Initialize and zero fill arrays */  
	...
	N			= (float *) calloc(FFTLEN, sizeof(float));	//noise buffer
	N_i			= (float *) calloc(FFTLEN, sizeof(float));	//noise buffer
	P			= (float *) calloc(FFTLEN, sizeof(float)); 	//low-pass filtered sample
