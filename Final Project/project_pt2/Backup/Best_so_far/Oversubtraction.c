...
int 	lpf_N_switch=1;
float 	tau=0.02;//Unit ms, range 20~80 ms, control weight of latest input, trade-off between voice and noise,0.04
float 	alpha=	2;//trade-off between speech and noise
float   lambda=0.13;//trade-off between background noise and musical noise, ALLOWS LOWER in this example
...
void noise_estimate(){
	...
	{                          
		...
		if((cabs(buffer[i])-N_i[i])>ratio*N_i[i]){
			alpha=2;
		}
		else{
			alpha=5;
