...
#define OUTGAIN 26000.0				/* Output gain for DAC */ //modified to make sound louder
...
float	tau2=0.08;
float 	alpha=	3;//trade-off between speech and noise
float   lambda=0.03;//trade-off between background noise and musical noise, ALLOWS LOWER in this example
...
void frame_in(){
	...
	if(i==0){
		...
		for(k=0;k<FFTLEN;k++){
			mag=cabs(buffer[k]);
			P[k]=(1-lpf_weight_speech)*mag+lpf_weight_speech*P[k];
			if(P[k]<M1[k]){
				M1[k]=P[k];
			}
		}
...
void noise_estimate(){
	...
	{    
		...
		SNR_GAIN=-0.2*SNR+5;
		if(add1==1){
		if(i<=5||(i>=80&&i<=176)||i>=251){s_factor=20;}
		}
